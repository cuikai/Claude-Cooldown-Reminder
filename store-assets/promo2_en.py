#!/usr/bin/env python3
# Chrome 商店截图 promo2_en.png — 1280x800，主打「5 小时窗口规划」
...
badge_txt = "Chrome Extension · Claude Cooldown Reminder"
bf = FB(15)
...
h1 = FB(44)
hy = by + 38 * S + 34 * S
text(LX, hy, "Your next 5-hour window,", h1, TEXT)
line2_y = hy + 59 * S
text(LX, line2_y, "planned", h1, READY)
text(LX + tw("planned", h1), line2_y, " ahead for you", h1, TEXT)

# sub
sf = FR(19)
sy = line2_y + 59 * S + 24 * S
text(LX, sy, "The light turns green the moment your quota refills,", sf, TEXT2)
text(LX, sy + 33 * S, "with your next three 5-hour windows mapped out.", sf, TEXT2)
...
feats = [
    ("check", True,  "Green means go",   "The instant the countdown hits zero, one message opens a new window"),
    ("cal",   False, "Window timetable", "Your next three 5-hour windows, start to finish, at a glance"),
    ("clock", False, "Anchored to now",   "The timetable follows the clock — start whenever, it stays accurate"),
]
fy = sy + 33 * S * 2 + 40 * S
...
text(LX + tw("★★★★★", star_f) + 12 * S, foot_y + 1 * S, "Free · English & Chinese · No sign-up", FR(14), TEXT3)
...
text(PX + 69 * S, PY + 16 * S, "Claude Cooldown Reminder", FB(19), TEXT)
text(PX + 69 * S, PY + 46 * S, "We'll ping you the moment your quota refills.", FR(14), TEXT2)
...
text(CARD_X + 41 * S, c1y + 17 * S, "Ready to go", FB(17), TEXT)

hf = FR(15)
hx, hy2 = CARD_X + 18 * S, c1y + 56 * S
text(hx, hy2, "Send a message now to start a new 5-hour window.", hf, TEXT2)

# window rows
wf = FR(15)
wtf = FB(14)
wy = hy2 + 38 * S
rows = [("1", "Today 14:10 → Today 19:10", True),
        ("2", "Today 19:10 → Tomorrow 00:10", False),
        ("3", "Tomorrow 00:10 → Tomorrow 05:10", False)]
...
text(CARD_X + 18 * S, c2y + 16 * S, "Usage limits", FB(16), TEXT)
lf = FR(13)
lw_ = tw("Refresh", lf) + 26 * S
rr([CARD_X + CARD_W - 18 * S - lw_, c2y + 14 * S, CARD_X + CARD_W - 18 * S, c2y + 14 * S + 27 * S], 9,
   outline=(255, 255, 255, 20), width=S)
text(CARD_X + CARD_W - 18 * S - lw_ + 13 * S, c2y + 17 * S, "Refresh", lf, TEXT2)
...
uy = c2y + 58 * S
usage = [("Session (5h)", "0%", 0, OK1, OK2, ""),
         ("Weekly", "26%", 26, OK1, OK2, "3d"),
         ("Fable weekly", "45%", 45, WARN1, WARN2, "3d")]
for name, pct_s, pct, cA, cB, reset in usage:
    ...
    uy += 62 * S
d = ImageDraw.Draw(img, "RGBA")
text(CARD_X + 18 * S, uy - 4 * S, "Updated 14:08", FR(13), TEXT3)

# ---- green chip floating above popup ----
chip_f = FB(15)
chip_txt = "Quota restored — ready to send"
...
out = f"{REPO}/store-assets/promo2_en.png"
